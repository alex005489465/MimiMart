...
# --------------------------------------------------------------------
# Bucket Policy - 限制 IP 存取
#
# 允許公開讀取，但僅限 Cloudflare IP + 開發者 IP
# 這確保了公開資源只能透過 CDN 或授權開發者存取
#
# IP 來源:
#   - Cloudflare IPv4 範圍 (15 個 CIDR)
#   - Cloudflare IPv6 範圍 (7 個 CIDR)
#   - 開發者 IPv4 (從 developer-ips.json)
#   - 開發者 IPv6 (從 developer-ips.json)
# --------------------------------------------------------------------
...
resource "aws_s3_bucket_policy" "public_cloudflare_only" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      {
        Sid       = "AllowCloudflareAndDeveloperIPs"
        Effect    = "Allow"
        Principal = "*"
        Action    = "s3:GetObject"
        ...
        Condition = {
          IpAddress = {
            # 包含 IPv4 和 IPv6
            "aws:SourceIp" = module.ip_ranges.cloudflare_and_developers
          }
        }
      }
    ]
