  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <h1 className={styles.title}>分類管理</h1>
          ...
          <table className={styles.table}>
            <thead>
              <tr>
                ...
                <th className={styles.nameHeader}>分類名稱</th>
                <th className={styles.descriptionHeader}>描述</th>
                <th>建立時間</th>
                <th>更新時間</th>
              </tr>
            </thead>
            <tbody>
              {categories.map((category) => (
                <tr key={category.id}>
                  ...
                  <td data-label="分類名稱" className={styles.nameCell}>
                    {category.name}
                  </td>
                  <td data-label="描述" className={styles.descriptionCell}>
                    {category.description ? (
                      category.description
                    ) : (
                      <span className={styles.noData}>無描述</span>
                    )}
                  </td>
                  <td data-label="建立時間" className={styles.dateCell}>
                    {formatDateTime(category.createdAt)}
                  </td>
                  <td data-label="更新時間" className={styles.dateCell}>
      ...
      <ConfirmModal
        ...
        title="確認刪除"
        message={`確定要刪除分類「${deleteModal.categoryName}」嗎？此操作無法復原。`}
        confirmText="刪除"
        cancelText="取消"
        isDanger={true}
